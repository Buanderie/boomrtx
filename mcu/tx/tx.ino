...
// Task no.1: Send PINGs into the AIR
  ...
  COROUTINE_LOOP() {
    while (__radioInterface->available()) {
      ...
      if( radioFrameParser.addByte( c ) )
      {
        ...
        if( f.opcode == OP_RX_PONG )
        {
          int offset = 0;
          uint8_t device_id = f.payload[ offset++ ];
          // Skip device type
          offset++;
          ...
          // Now grab the fire states
          uint8_t num_active_relays = f.payload[ offset++ ];
          bool targetRelayActive = false;
          for( int i = 0; i < num_active_relays; ++i )
          {
            uint8_t relay_slot = f.payload[ offset++ ];
            if( __targetRelay == relay_slot )
            {
              targetRelayActive = true;
              break;
            }
          }
          ...
          {
              bool isActive = false;
              if( i == __targetRelay && targetRelayActive )
              {
                isActive = true;
              }
              Frame fireAck = createFireAckFrame( device_id, i, isActive );
              int bsize = frameToBuffer( fireAck, buf, 32 );
              __radioInterface->write( buf, bsize );
          }

          if( targetRelayActive )
          {
            // Light up RED
            if( __fireLEDON != 1 )
